...
<div class="container mt-4 bg-white p-5">
    <h1 class="mb-3">Nou grup</h1>
    <a href="{{ route('grup_list') }}" class="btn btn-secondary mb-4">&laquo; Torna</a>

        <form method="POST" action="{{ route('grup_new') }}" enctype="multipart/form-data" class="row g-3">
            @csrf
            <div class="col-md-6">            
            ...
            <div class="col-md-6">                            
                ...
                <input type="text" name="aula" id="aula" class="form-control @error('aula') is-invalid @enderror" value="{{ old('aula') }}" />
                    @error('aula')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            </div>
                    ...
                    <select name="tutor" id="tutor" class="form-select @error('tutor') is-invalid @enderror">
                        ...
                        @foreach ($professors as $professor)
                            <option value="{{ $professor->id }}" 
                                @selected(old('tutor') == $professor->id)>
